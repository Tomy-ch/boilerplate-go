...
|スクリプト|説明|実行元|
|---|---|---|
|`realtime-reset/`|この checkout の Realtime Delivery の 3 table を削除し、消え切るまで待つ。スロットの PostgreSQL と DynamoDB EventLog を一緒に作り直すためである。`slot-acquire` が PostgreSQL しか作り直さない穴を埋める: 採番が 1 から再開する一方 EventLog には同じ位置の item が残っており、条件付き書き込みはその衝突を — 正しく — 拒否するので、relay の head-of-line blocking の裏で stream が止まったままになる。table 名は app が読むのと同じ `REALTIME_TABLE_SUFFIX` から取るため、serve が使う名前とずれようがない。作成は `realtime-init` の担当のままで、こちらは削除だけを行い、無い table は成功として数える。空の `-endpoint` を拒否するのは SDK 既定の解決へ落ちて本番 DynamoDB へ届くためで、消えるまで待つのは後続の `realtime-init` が `ResourceInUseException` を踏まないようにするためである。|`make realtime-reset` / `make slot-acquire`|
