...
`AGENTS.md` はエージェントに `*.ja.md` を読むなと言っている。あの規則が言っているのは、翻訳を知識の出所として使うなということである。翻訳は原文に遅れるので、そこから考えると古い答えが出る。読むべきは canonical の英語版である。

このスキルの subject は対訳ペアそのものであり、あの規則が当てはまらない唯一の場合である。更新する側を読めない同期は、目隠しで上書きする（そこに既にある訳語と、2 つのファイルが共有している構造を捨てる）か、作業を差し戻すかしかない。後者は、1 世代遅れた翻訳の隣に `SKILL.md` が出荷される道である。

したがってこの実行の間、かつ最初に確認したペアに限り、`*.ja.md` は自由に読んでよい。変更がどこに落ちるかを特定するため、そして既に使われている訳語に合わせるためである。それ以外は動かない。canonical は英語のままであり、翻訳がシステムに関する事実の出所になることは無く、確認したペアの外の `*.ja.md` は開かない。
