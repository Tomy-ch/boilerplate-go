...
name: full-apply
description: >-
  The "apply" skill that applies = fixes the findings under tmp/skills/reviews/ generated by full-verify (architecture.md / mod_*.md / _index.md), top-down by severity (Critical → High → Medium → Low). The counterpart to the read-only full-verify. Skips (defers) "suspicious" findings that require design judgment or whose behavior is unclear, leaving a reason. Tracks done/deferred in the tmp/skills/reviews/working.md ledger and stamps a status comment + commit hash at the top of each mod_*.md. Conflicts are first-come-first-served; within one md, related findings are fixed together within the bounds of not breaking the public API. Verifies (build/vet/test/gofmt or make) before committing. Use it when asked to "fix the review findings", "apply reviews", or "full apply".
argument-hint: '[--reviews-dir tmp/skills/reviews] [--severity critical|high|medium|low] [--scope <dirs csv>] [--pace dir|file|all] [--dry-run]'
allowed-tools: Read, Grep, Glob, Edit, Write, AskUserQuestion, Bash(grep:*), Bash(git rev-parse:*), Bash(gofmt:*), Bash(go build:*), Bash(go vet:*), Bash(go test:*), Bash(make go-fix:*), Bash(make go-lint:*), Bash(make go-test:*), Bash(mise install:*), Bash(mise exec:*)
---
...
- Fixes use `internal/` `pkg/` `database/` `openapi/` as the base scope, but if the user explicitly
...
2. **Check go toolchain connectivity** (this repository is Go / mise-managed). `make go-fix` / `make
   lint` / `make go-test` are the canonical paths. If `go`/`make` fails due to a broken GOROOT or version
...
- **Target directories**: the default is CLAUDE.md's AI modification scope (`internal/` `pkg/`
...
If possible, the canonical `make go-fix` → `make go-lint` → `make go-test`. If a test depends on a specific
