...
description: >-
  Implement the controller (HTTP handler) layer for one feature, derived from the OpenAPI-generated `ServerInterface` and the target usecase Interface (lean A — no spec file). It maps each operationId to a usecase method, halting with a hand-off message on any it cannot map, generates the handler, its constructor and test, registers it in the DI module, and chains `scaffold-integration-test`. Prerequisites: the OpenAPI YAML is written and `make gen-api` has run, and the target usecase is implemented with its mock. Use it when the domain, infra and usecase layers exist and the HTTP layer is what remains. Standalone-callable; the last scaffold step when chained from `scaffold-endpoint`. Do NOT use it to design the API contract itself (write the OpenAPI first) or to implement any other layer (`scaffold-domain` / `scaffold-infra-db` / `scaffold-usecase`).
---
...
This skill **MUST call `AskUserQuestion` immediately after invocation** (unless invoked from `scaffold-endpoint` with feature + handler path + usecase package in context):
...
5. Read 1 sibling handler (`internal/controller/handler/v1/<sibling>/v1_<sibling>_handler.go` etc.) as structural template. <!-- skill-lint-ignore -->
...
Invoke the Agent tool to enumerate controller-layer test viewpoints BEFORE writing code.
...
- ❌ Skip identity confirmation `AskUserQuestion`
