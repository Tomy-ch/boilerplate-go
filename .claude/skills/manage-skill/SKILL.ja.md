...
- スキルの `description` のトリガー精度最適化、本文の再掲に膨らんだ description の圧縮、本文と食い違って腐った description の正規化。
- スキルの eval / benchmark 実行。
...
- **`description` の収録基準は「呼び出し判定に効くか」であって「網羅」ではない。** 各スキルの `description` は、そのスキルが呼ばれるかどうかに関わらず毎セッションの冒頭で全文読み込まれる。だから一文が場所を得る条件は *このスキルを呼ぶべきかの判断を助けること* だけ。収録するのは 5 つ — 何をするか、いつ使うか（日本語のトリガ語を含む）、いつ使わないか＋その主題を持つ兄弟スキル、呼び出し側が満たしておくべき前提、ユーザーが依頼時に口にするフラグ。それ以外は本文の担当で、本文はスキルが実際に走るまでコストゼロ — 内部機構（サブエージェント名・既定モデル・fan-out 構成・パイプライン段数）、スキルが強制するルール、検証に使うコマンド、出力フォーマット、そしてそれらの根拠。「密に」はトリガー面の話であって、本文の再掲のことではない。
- **`description` から一文を消す前に、本文が既にその内容を持っていることを確認する。** 両者が乖離するのは、ある事実が片方にしか書かれなかったからで、実は移設だった削除は指示の消失になる。
- **スキルに触れたら、腐った `description` をその場で正規化する。** `description` はスキルの実行中に読まれない唯一の部分なので、嘘を書いても何も落ちず、機械的なゲートも無い。本文と、そこで名指ししている兄弟スキルに対して、記述の一つ一つを突き合わせること — もう存在しない chain 関係、改名・削除されたレンズ / モード / フラグ、別スキルが引き取った主題、移動したパス。issue 化ではなく同じ編集で直す。
- 上記 2 つは **一括掃引** にも同じく適用する。目の前のスキルだけでなく、description が溜まってきたら `.claude/skills/*/SKILL.md` 全体に対して回すこと。削減はセッション単位かつリポジトリ全体に効き、基準は同一のものをファイルごとに適用するだけ。
...
スキルは指示書であり、その著者だけが唯一それを監査できない読者である。指示書が省いている文脈を著者は
持っているので、読みながら頭の中で穴を埋めてしまい、穴そのものは見えない。草稿を読み返しても、レビューに
出しても、この工程が捕まえる失敗が表に出ないのはそのためである。

**答えを自分が知らない実問題を 1 件**取り、手順を省略せずに通す。答えを知らないことが効く —— 知っている
問題では記憶で走り抜けてしまい、欠けている欄は自分から名乗り出ない。

そのうえで、**契約に欄が無いものを書いた箇所と、契約が要求しているのに書けなかった箇所**をすべて記録する。
それが欠陥である。このリポジトリで新設した 5 スキルで実測したところ、この方法で出た穴は**すべて出力契約の
欠落**で、読んでいる間に見えたものは 1 つも無かった:

- 必須の判定はあるのに、それを支える証拠を書く欄が無く、実行時に欄を発明していた
- ある状態（`UNDEFINED`）のとき、隣の欄に何を書くかの規則が無かった
- 分析から軸を 1 つ落とす実行モードがあるのに、落としたと述べる場所が無かった

同じ形の穴が複数のスキルに出たら、**個別に当てるのではなく欄の形を横に揃える**。繰り返す穴は規約の欠落で
あり、3 通りに直せば 1 つの family として読めない 3 つの契約ができあがる。

これはスキル自体の検証であって、出力品質の検証ではない。後者は下の公式 eval ループが測るもので、2 つは
別の問いに答えている。
...
- `description` が §2 の収録基準を満たす。呼び出し判定に効かない記述が無く、本文が裏付けない主張が無く、本文が持っていない内容を削っていない。
...
- 答えを自分が知らない実問題で 1 回通し、契約に欄が無かった箇所・要求されているのに書けなかった箇所を
  すべて直した。
