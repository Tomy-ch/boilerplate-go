...
周囲のコメントを 1 つの総体として読んだときにしか現れないので、どれだけ丁寧に走らせても per-comment のパスでは
見つからない。

そこで、どの監査も**自分に割り当てられたパッケージ全体**のコメント在庫に対して 2 つ目の問いを立てる:

> **その内容は、このパッケージの他の宣言に既に書かれていないか。書かれているなら、どの 1 箇所が
> 所有者か。**
...
- **総量過多** — 各コメントは個別には正しいのに、パッケージのコメント総量が、説明対象のコードより読むのに
  高くつく。

これは diff スコープの罠の 1 階層下にある同じ罠である。孤立して問われるかぎり各コピーを残す主張が毎回
勝ち、何も集約されない。集合に対して問え。

### 3 つ目の問い: 1 パッケージずつでは見えないもの

罠はもう 1 階層外側で再発する。2 つ目の問いをファイルではなくパッケージに対して立てる理由がこれである。
**パッケージをまたいで**繰り返された Why は、どの監査でも単独で 2 つ目の問いを通ってしまう。各監査は
自分のスコープしか見ないので、関係を見ている者が誰もいない。

これは仮定の話ではない。このリポジトリの掃引で、同一の文が一字一句同じ形で 3 パッケージ 6 宣言に
現れていた。各監査は自分が抱える 2 件ないし 4 件を別々の finding としてしか報告できず、その文を書いた
実行は、6 回書くあいだ一度も「これはコードに置くべきか」を問われなかった。

**全パッケージを見るのは統合側だけなので、3 つ目の問いは統合側のものである**（Step 2.5）。判断ではなく
機械的に問う。監査は 維持 を件数でしか報告しないので、その内容は報告をまたいで比較できないからである:

> **監査が別々に判定する複数の宣言に、同じコメント行が現れていないか。**

こうして見つかった塊は、自動的に 集約 になるわけではない。まず管轄で解く。答えはパッケージ内の場合とは
たいてい違う:

- **繰り返された内容の管轄が文書である** — このとき**各サイトで 移設**（文書が既に述べているなら 短縮）に
  なる。パッケージをまたぐ概念を所有する宣言は存在しないので、集約する先が無い。塊を検出して得られるのは、
  N 件の独立した「維持」判断が 1 つの見える判断になることである。
- **1 つの宣言が本当に概念を所有していて、他がそれを名指しできる** — このときメンバーがファイルをまたぐ
  集約になる。ポインタは所有する宣言を名指ししなければならない。別パッケージの読み手は近接では辿れない。

明示しておくべき限界: 機械的な走査が見つけるのは繰り返された**行**なので、逐語の反復は捉え、言い換えは
取り逃がす。逐語が支配的な形であり（同じ文はコピーされるのであって導出し直されない）、言い換えを
見つけると称さない走査のほうが、誰も実行しない判断より役に立つ。
...
走らせる — コメント単位の管轄の問いと、パッケージ単位の在庫の問いである。2 つ目のパスに追加の読み込みは
発生しない。増えるのは問いであり、そこから出る finding（判定 **集約**）は 1 件のコメントではなくコメントの
*集合*を名指す。Step 2.5 で見つかったパッケージ横断の塊のうち、その監査のファイルに掛かるものは渡す。
他所でも繰り返されていることを知ったうえで判定させるためである。
...
## Step 2.5 — パッケージ横断の繰り返しを走査する（統合側・機械的）

fan-out と同じメッセージで、監査の報告が返る前に実行する。*なぜこのスキルが要るのか* の 3 つ目の問いで
あり、ここに置くのは、どの監査も他の監査のスコープを見られないからである。

対象ファイル全部のコメント行を集め、行頭のマーカーとインデントを落とし、節に満たない短い行を捨てて、
**複数のファイル**の宣言に現れるテキストを報告する:
...
`grep` は効いている部品である。これが無いとコード行や空行まで塊にしてしまい、毎回 1 つの巨大な無意味な
塊を報告する。仮定の話ではなく、この Step の最初の草稿が実際にそうだった。

パイプラインの厳密さより性質が重要である。**決定的で安価**なので、誰かが重複を疑ったときではなく毎回の
掃引で走る。長さの下限はスコープに合わせて調整する — 低すぎると定型のフィールドコメントが支配し、
高すぎると 1 行の Why がすり抜ける。

各塊は *3 つ目の問い* の規則で解く。まず管轄（たいていは各サイトでの 移設 / 短縮）、1 つの宣言が本当に
概念を所有している場合にかぎりファイル横断の 集約。メンバーが 1 パッケージに収まる塊はそのパッケージの
監査へ渡し、残りは統合側が抱える。

**メンバーが全部個別には正しくても、塊は finding である。** それこそが要点で、各コピーは既に単独で管轄を
通っている。だから誰も気づかなかった。
...
- **集約は `確度: high` かつメンバーが同一ファイルに収まるときだけ適用する。** 短縮が誤るのは 1 箇所の
  文言だが、集約はそれに加えて*どの宣言が概念を所有するか*を選ぶ判断であり、選択の誤りが表面化する頃には
  他の site は既に縮められている。取り消しは目に見えて難しくなるので、監査エージェント自身が `medium` /
  `low` と評価したものは適用せず確認して適用へ回す。メンバーがファイルをまたぐ集約を保留するのも同じ理由の
  1 階層上である。ポインタは近接では辿れない宣言を名指しする必要があり、その宣言が本当に所有者かどうかは、
  質問しないモードには下せない判断である。

確認を通った `追記なし` の移設は適用する — 移設先に既に同内容があるので、実体は残滓＋リンクへの短縮であり、
文書には触れない。同一ファイルの集約も同様に文書を書かない — コメント間で内容を動かすだけである。
...
- 集約の後は、編集した各 site を個別に見るのではなく、触れたファイルを通しで読む。finding がコメントの
  総体についてのものだった以上、検証もそうでなければならない。この読み方でしか出てこない失敗が 2 つある:
  本体側が縮めた側の手放した内容を実際には引き受けていないこと、そしてポインタが、読み手の現在地からは
  辿れない宣言を指していることである。メンバーがパッケージをまたいだ場合は、**別パッケージ側から**
  ポインタを読むこと。所有する宣言の隣では自明な参照が、3 ディレクトリ離れると辿れないことは多い。
...
この独立性が、掃引を在庫単位に保っている当のものでもある。diff を渡してくるものが無く、監査エージェントが読んでよいコメントを絞るものが無く、在庫のパスが見る前にコメントを間引くものも無い — だからコメントとコメントの*あいだ*に住む重複が、1 ファイルの中だろうと 1 パッケージの中だろうと 3 パッケージにまたがろうと、見えたままになる。変更箇所だけを受け取る掃引は、「在庫」という語をまとった 2 度目の diff レビューにすぎない。
...
| **`comment-sweep`**（本スキル） | **コメント 1 件、パッケージの在庫全体、そしてパッケージ横断の繰り返し** | **維持 / 短縮 / 削除 / 移設 / 集約** | **管轄 — 内容がどこに属し、どの 1 箇所が所有するか** |
