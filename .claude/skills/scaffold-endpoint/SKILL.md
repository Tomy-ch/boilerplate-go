...
description: >-
  End-to-end orchestrator that builds a complete onion-architecture endpoint (domain + infra-db + usecase + controller) for one feature, and — when you start from a rough idea instead of finished specs — first drives the upstream design phases (clarify, explore, design) that turn that idea into the OpenAPI YAML, SQL migration and specs the scaffold core needs. Both entry modes are auto-detected: idea-first, and specs-ready. Use when starting a new feature / endpoint end-to-end, when you have only an idea or a requirement and no specs yet, or when you want the whole controller-usecase-domain-infra stack built consistently with one consolidated report. Halts (never auto-rollbacks) on any failing phase; each layer keeps its own human-in-the-loop confirmation. Do NOT use it for modifying a single existing layer (run the specific `scaffold-<layer>` standalone), for a pure spec-template scaffold (`new-spec`), or for a review-only pass (`impl-review` / `arch-check` / `test-review`).
---
...
**MUST call `AskUserQuestion` immediately after invocation**, one batched call:
...
- **Mode A**: additionally, the upstream phases draft input artifacts in Phase 4 — only under `openapi/**`, `database/dml/**`, `database/migrations/**` (new files only), and `docs/spec/domain/**` / `docs/spec/usecase/**` (all inside the `CLAUDE.md` AI modification scope). It never writes Go source and never touches generated files.
