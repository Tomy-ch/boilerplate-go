...
| 段 | 実行モデル | 出力 |
| --- | --- | --- |
| 3a 観点整理 | 実装者と異なるモデル（既定 `fable`） | 計画が答えるべき問いだけ。それ以外は出させない |
| 3b 情報収集・起草 | それが最も得意なモデル（既定 `opus`）を subagent として | 計画ファイル |
| 3c 計画レビュー | 3a と同じモデル | 指摘。計画ファイルへ独立した節として追記する |
...
- **3b のモデルが実装者と同じ**（通常のケース。Opus セッションが `opus` で起草した場合）。計画はまだ他のどのモデルの目も通っていない。**3c は必須。**
- **3b のモデルが既に実装者と異なる**（たとえば `fable` セッションが `opus` で起草した場合）。3b が終わった時点で不変条件は満たされており、3c は任意 — 二度目を通す価値がある規模なら回し、無ければ省く。
...
- [ ] Step 3a〜3c で計画を作成し、3a の問いすべてに答えがあり、必須 4 セクションが揃い、実装者と異なるモデルの目を通した上で、実装前に承認された。
