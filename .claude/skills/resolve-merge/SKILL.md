...
name: resolve-merge
description: >-
  Land a merge correctly by classifying every conflicted path into a resolution class and applying the mechanical resolution each class already has — regenerate generated artifacts from their source of truth rather than picking a side, re-resolve pin lockfiles through their resolver, union append-only registries, propagate version derivations from `mise.toml` — then hand back only what genuinely needs a human. Use it whenever a merge reports conflicts, and equally whenever one reports none: several committed artifacts here are concatenations or derivations that drift on a clean merge, so "no conflict" is not the same as "nothing to do". It takes the base from the pull request's own `baseRefName` and merges rather than rebases, per `CLAUDE.md`. Do NOT use it to resolve a semantic conflict in implementation code — it hands those back untouched — to rebase or squash anything, to decide which of two colliding registry keys wins, or to sync a branch nobody asked to sync.
...
`make go-lint` / `make go-test` may be deferred to CI per this repository's local-gate load bands; say which
