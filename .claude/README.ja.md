...
`rtk` はシェル出力をエージェントのコンテキストへ届く前に圧縮する。バージョンは mise が解決する他の
ツールと同じく `mise.toml` がピンする（`mise install rtk`）。build / test / CI のどの経路も呼ばないため、
これが無い checkout でも挙動は変わらない。変わるのはエージェントのコンテキスト量だけである。

### 削減が出るもの・出ないもの

- **`rtk read` は既定では何も削減しない。** 既定の `--level none` は全文であり、削減は行を落とす
  `-l minimal` / `-l aggressive` から来る。ソースをその形で読んではいけない。欠損しているのに
  完全に見えるファイルは、長いファイルより判断を誤らせる。
- **`rtk diff <rev>` は空を返す。** 受け取るのはファイルパスであって git の rev ではない。
  `rtk git diff` を使う。
- **`go test` / `golangci-lint` はここでは未実測。** どちらも `make` 経由で tool-runner の中で走り、
  `rtk` は実際の呼び出しを見ない。上流のフィルタが説明しているのは素の呼び出しである。
- **決定論的な検査の結果を、欠損するフィルタ越しに報告してはいけない。** `AGENTS.md` は test / lint /
  gate が何と言ったかを報告することを要求する。`rtk` は失敗だけを見せ、件数と網羅率を落とす。
  数値そのものが主題のときは `rtk run <command>` で生出力を取る。

### マシン側のセットアップと、ピンが届かない理由

自動書き換えフック（`rtk init -g`）と除外リストはユーザーのホームディレクトリにあり checkout の中には
無いため、どちらもリポジトリと一緒に配布されない。帰結は 2 つある。

- **フックは `PATH` から `rtk` を解決するため、`mise.toml` のピンに従わない。** 別途入れた `rtk` は
  ピンされた版と食い違う。バージョンが問題になる場面ではピンされたビルドを呼ぶ。
- **除外の基準は出力が冗長かどうかではなく、出力を厳密な値として読むかどうかである。** このリポジトリが
  除外しているものとその理由:

  | 除外 | 理由 |
  | --- | --- |
  | `gh` | `--json` の出力が判断を左右する（`mergeable` / `baseRefName` / `state`） |
  | `make` | 出力そのものが値になる target がある（`base-branch` / `slot-status`）。かつ tool-runner が実際のツールを `rtk` から隠すので、除外の損失はゼロ |
  | `psql` | 検証対象は行の値そのもの |
  | `curl` | API のレスポンス本文を spec に照らして検証する |
