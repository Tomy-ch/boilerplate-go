...
This skill **MUST call `ask the user explicitly` immediately after invocation** to confirm the target version.
...
2. **Always** invoke `ask the user explicitly` to ask the user:
...
This step **MUST call `ask the user explicitly`** to confirm the update level. Offer:
...
- [ ] Target version `<TARGET_VERSION>` confirmed with the user via `ask the user explicitly`
