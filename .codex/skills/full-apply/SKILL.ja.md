...
変更は一貫性を保てる最小の単位にする。同一ソースファイル内の関連する findings をまとめて処理してよいのは、それが挙動の変更範囲を広げない場合だけである。バッチをコミットする前に、関係する範囲で最も狭い整形・ビルド・vet・テストのコマンドを実行する。適切な場面では `make go-fix` / `make go-lint` / `make go-test` を用いる。
