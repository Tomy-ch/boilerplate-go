...
1. `make gate-fix` を 1 回実行する。これは全コミットで走る一方、`fix` は `lint` と同じフル設定の lint を使うため、負荷帯が `ci-first` のときは委譲され、整形のずれは CI の lint が報告する（素の `make go-fix` は帯に関わらず無条件で走る）。帯が委譲した場合、変更が出ないのは設計どおりであって、ツリーが既に整形済みだったことを意味しない。失敗したら止める。生じた変更は候補集合に含める。
...
フックは自分で規模を決める。`.makefiles/load.mk` が開いている worktree の数から、重い Go ゲートを全速で走らせるか、絞るか、CI へ委譲するかを決定する。現在の帯は `make load-status` が報告し、`repo-ops` の §19 が方針を説明している。**この決定を迂回して `make go-lint` や `make go-test` を直接叩き、「本当に」検証しようとしないこと。** 窓が複数開いているとき、フルのローカル lint は、CI が同じものを走らせ直すだけの内容を再発見するために、飽和したホストを数分間占有する。帯が何をしたかを報告し、残りは push に運ばせること。
...
5. 自動で push しない。既存 PR のブランチでは、コミットがローカルにあることを報告し、「変更はローカルにコミット済みです。これらの変更をプルリクエストにプッシュしますか？」と正確に尋ね、ユーザーの返答を待って止まる。force push・rebase・保護ブランチへの直接 push は決して行わない。

作成したコミット・検証結果・残った変更・push を行っていないことを報告する。帯が `ci-first` だった場合は、どのゲートが委譲されたか、そしてそれらを検証するのは CI であることを明確に述べる。**その但し書きなしに「検証が通りました」と言わないこと。**
