...
後段のゲートは計画を再検討しない。`impl-review` / `test-review` / `comment-sweep` はいずれも完成した変更を対象にするため、その計画で issue が本当に解決するかを確認できるのはここだけである。良い下書きを作る仕事と、偏りなく下書きする仕事は別なので、別々の段階として実行する。**3 段階は停止箇所を増やさない。** 最後の承認が唯一の待機である。

| 段階 | 実行モデル | 成果物 |
| --- | --- | --- |
| 3a — フレーミング | 実装者とは異なるモデル | 未解決の問いだけ |
| 3b — 調査と下書き | 調査と下書きに最も強いモデル。サブエージェントへ委譲する | 計画ファイル |
| 3c — 計画レビュー | 必須または有益な場合に、3a と同じモデル | 独立セクションとして追記した指摘 |

### 3a — フレーミング

Codex のエージェント委譲機構を使い、実装者とは異なるモデルへ issue 本文、Step 1 の issue ↔ ベースの食い違い、既に読んだパスを渡す。出力は**未解決の問いだけ**、すなわち計画が答えるべき問いだけとする。この段階から計画案、推奨、方向性が返ったら拒否する。この時点ではリポジトリをほとんど読んでいないため、断定できる内容は一般論にすぎない。そして、強い下書き担当へ渡した一般論は視野を広げるのではなく、アンカーになる。

### 3b — 調査と下書き

調査と下書きに最も強いモデルのサブエージェントへこの段階を委譲する。調査を、オーケストレーターが蓄積したフレーミングを一切持たないウィンドウで行うためである。issue、Step 1 の訂正、既に読んだパス、3a の問いを渡す。「私の要約を鵜呑みにせず自分で検証しろ」と伝える。3a の**すべて**の問いに対し、どう判断したか、またはこのケースには該当しないことを必ず答えさせる。
...
この段階が必須かどうかは導出し、決め打ちしない。3b のモデルが実装者のモデルなら、まだ他のモデルは計画を見ていないため 3c は必須。3b のモデルが実装者と異なるなら、3b の完了時点で不変条件を満たす。変更が second pass に値するほど大きければ 3c を実行し、そうでなければ省略する。3c は 3a と同じモデルで実行する。指摘は計画ファイルへ独立したセクションとして追記し、計画へ織り込まず、その横に並べて提示する。レビュアーが黙って計画を書き換えると、ユーザーに承認を求めるその瞬間に不一致が隠れる。承認時に各指摘を裁定するのはユーザーである。

現在の Codex surface が別モデルのエージェントを起動できないなら、3a と 3c のどの段階を別モデルで実行できなかったかを明示する。3a の問いだけという契約を守って自分で実行し、可能なら 3b を委譲し、できなければ自分で下書きする。その後、3c として独立した自己批評パスを行い、上記どおり指摘を追記する。この fallback は不変条件を満たさないと明記する。不変条件を黙って落としたり、自己批評を Step 3 全体の代わりにしたりしてはならない。
...
- [ ] 3a–3c で計画を構築し、3a の全質問へ回答し、必須 4 セクションが揃い、実装者とは異なるモデルが確認し、実装前に承認された。
