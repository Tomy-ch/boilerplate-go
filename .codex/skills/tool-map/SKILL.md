...
This command inventories every customization entry under the **project-level** `.codex/` directory (it does NOT scan `~/.codex/`). It covers three types — commands, skills, and agents — and produces a single Markdown report.

## Step 1. Resolve Inputs

Parse `$ARGUMENTS` for the following optional flags. For each flag missing or invalid, fall back to `ask the user explicitly` to confirm.
...
`ask the user explicitly` fallback questions (only ask for unresolved flags):
...
- Scan scope is **project-level only**. Do NOT read or list anything under `~/.codex/`.
- Plugin-provided entries are out of scope.
- Do NOT modify any scanned entry. This command only inspects.
- If `.codex/commands/`, `.codex/skills/`, or `.codex/agents/` does not exist, treat its entry count as 0 and note it in the report rather than erroring.
