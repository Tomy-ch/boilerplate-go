...
`ask the user explicitly` を起動直後に必ず呼ぶ（`scaffold-endpoint` から呼ばれて context に aggregate 名がある場合は除く）:
...
Codex delegation を起動して infra 層 test 観点を実装前に列挙:
...
- ❌ identity 確認 `ask the user explicitly` スキップ
