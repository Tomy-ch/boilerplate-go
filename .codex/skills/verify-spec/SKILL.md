...
name: verify-spec
description: >-
  Integrator skill for spec validation. Confirms the target package path (or receives it from `scaffold-endpoint`), then validates whichever of `docs/spec/domain/<pkgpath>.md` / `docs/spec/usecase/<pkgpath>.md` exist — format, internal consistency, entity-to-SQL correspondence for domain, cross-spec references and naming convention for usecase — and reports in Japanese. It does NOT check OpenAPI operationId coverage: that would violate dependency direction, and `scaffold-controller` verifies the mapping at scaffold time. Read-only. Use it before implementing from a spec, or after editing one. Do NOT use it to write a spec (`new-spec`) or to audit implementation code (`arch-check`).
---
...
Validation is delegated to two **read-only worker subagents** under `.codex/agents/`, one per spec file. The integrator runs them concurrently via the Codex delegation (`subagent_type`):
...
For the existing spec files, spawn the matching validators with the **Codex delegation**, all in **a single message with multiple tool calls** so they run concurrently. Pass each validator:
...
Strictly read-only. The integrator and all validator subagents touch no spec or source files. The integrator only runs `ask the user explicitly` (package-path confirmation, standalone) and spawns read-only validators.

## Constraints

- ❌ validator を逐次起動（必ず1メッセージ内で複数 Agent 呼び出し＝並列）
- ❌ Hardcode rules — validators read `.codex/scaffold-spec/<layer>-spec.md` + `verify-rules.md` every run
- ❌ Auto-fix violations / modify any file
- ❌ Skip the target-confirmation `ask the user explicitly` (unless supplied by `scaffold-endpoint`)
...
- [ ] Target package path confirmed via `ask the user explicitly` (or supplied by `scaffold-endpoint`)
