...
func Test_lockByIDsSerializesConcurrentUpdates(t *testing.T) {
	...
	var (
		...
		holderReleasedAt = make(chan time.Time, 1)
	)
	...
	<-holderEnd
	<-secondDone
	// 後続役が完了したのがロック待ちの結果であることを、取得時刻の前後で裏づける。
	assert.True(t, (<-secondAcquiredAt).After(<-holderReleasedAt))
}
