...
func (u *usecase) IssuePromotionalCoupons(
	...
	now := u.clock.Now()
	// 有効期限は coupon.New も検証しますが、受給者が 0 人だと 1 枚も構築されず検証に届きません。
	// ここで見ておかないと、同じ要求が母集団の大きさによって 422 と 200 に分かれます。
	if !params.ExpiresAt.After(now) {
		return IssuePromotionalCouponsView{}, xerrors.Wrap(
			coupon.ErrInvalidExpiresAt, "expiresAt must be after the issuing time",
		)
	}

	if err = u.ensureScopeTargetExists(ctx, scope); err != nil {
		return IssuePromotionalCouponsView{}, err
	}
...
// newDiscount は、種別の名前と値からドメインの値引きを組み立てます。
// 未知の名前は ErrInvalidDiscountKind を返します。
func newDiscount(kind string, value decimal.Decimal) (coupon.Discount, error) {
	switch kind {
	case coupon.DiscountKindFlat.Name():
		return coupon.NewFlatDiscount(value)
	case coupon.DiscountKindRate.Name():
		return coupon.NewRateDiscount(value)
	default:
		return coupon.Discount{}, xerrors.Wrap(coupon.ErrInvalidDiscountKind, "unknown discount kind: "+kind)
	}
}

// newScope は、種別の名前と対象 ID からドメインの適用範囲を組み立てます。
// 未知の名前は ErrInvalidScopeKind を返します。
func newScope(kind string, targetID *uuid.UUID) (coupon.Scope, error) {
	switch kind {
	case coupon.ScopeKindAll.Name():
		if targetID != nil {
			return coupon.Scope{}, xerrors.Wrap(coupon.ErrInvalidScopeTarget, "an all scope must not have a target")
		}

		return coupon.NewAllScope(), nil
	case coupon.ScopeKindCategory.Name():
		if targetID == nil {
			return coupon.Scope{}, xerrors.Wrap(coupon.ErrInvalidScopeTarget, "categoryID is required for a category scope")
		}

		return coupon.NewCategoryScope(*targetID)
	case coupon.ScopeKindProduct.Name():
		if targetID == nil {
			return coupon.Scope{}, xerrors.Wrap(coupon.ErrInvalidScopeTarget, "productID is required for a product scope")
		}

		return coupon.NewProductScope(*targetID)
	default:
		return coupon.Scope{}, xerrors.Wrap(coupon.ErrInvalidScopeKind, "unknown scope kind: "+kind)
	}
}
