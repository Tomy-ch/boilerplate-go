...
- **Compose**（`Compose` / `ExecCompose`）— 共有 DB を固定 infra プロジェクトで起動（`--wait --no-recreate`）し、release 時にスロットの app プロジェクト（`gobp-wt-N`）を `docker-compose.yaml` + `docker-compose.attach.yaml` で、app 層のサービスが属する `development` profile を付けて停止する。profile を渡さない `down` は対象を 1 つも選ばずに exit 0 で返るため、成功を報告した release のあともコンテナが生き残る。`--no-recreate` は、他の checkout が使っているコンテナを `acquire` が置き換えないためのもの。ここで条件を持たない理由は [`docs/maintenance/db-worktree-pool.md`](../../../docs/maintenance/db-worktree-pool.md) を参照。
