...
- **Compose**（`Compose` / `ExecCompose`）— 共有 DB を固定 infra プロジェクトで起動（`--wait --no-recreate`）し、release 時にスロットの app プロジェクト（`gobp-wt-N`）を `docker-compose.yaml` だけで、app 層のサービスが属する `development` profile を付けて停止する。profile を渡さない `down` は対象を 1 つも選ばずに exit 0 で返るため、成功を報告した release のあともコンテナが生き残る。app 層の override をこの `down` に重ねないのは意図的である。override が変えるのは `api_server` の ports / `depends_on` / environment だけで、`down` はそのいずれも参照しない。重ねたままにすると、この片付け経路が compose の癖に依存することにもなる。override の `REALTIME_*` は値を必須（`:?`）とし、それを撒くのは `make` の `COMPOSE_APP` だけだが、それでもここで `down` が成功するのは、プロジェクトを `COMPOSE_PROJECT_NAME` で渡しているためである。この形の `down` はプロジェクトラベルだけで動きファイルを補間しないが、同じコマンドを `-p` で渡すと補間して失敗する。この非対称はどこにも書かれていないので、後から `newComposeCmd` を明示的な `-p` 形へ書き換えるだけで `slot-free` が壊れる。読まれる場所にだけファイルを渡せばその罠は消える。補間を通すためだけにダミー値を注入する案は却下した。`down` はその値を一切読まないためである。`--no-recreate` は、他の checkout が使っているコンテナを `acquire` が置き換えないためのもの。ここで条件を持たない理由は [`docs/maintenance/db-worktree-pool.md`](../../../docs/maintenance/db-worktree-pool.md) を参照。
- **スロットファイル** — `acquire` が `.gobp-db-slot`（gitignore・`make` が `-include` で読む `KEY=VALUE`）を書き出し、`.makefiles/docker/compose.mk` の既定値を上書きする。内容は `SLOT`、`DB_NAME_LOCAL` / `DB_NAME_TEST`、スロット相対のホスト公開ポート `API_HOST_PORT` / `MOCK_AUTH_HOST_PORT` / `DLV_HOST_PORT` / `PPROF_HOST_PORT`、`COMPOSE_PROJECT_NAME`（共有 infra プロジェクト）、`SERVE_PROJECT`（`gobp-wt-N`＝app 層プロジェクト）。このファイルはリースの証明ではない。stale として回収されたスロットの前保持者にも残り続けるため、`Resolver` も `release` も、ファイルの宣言に従って動く前にレジストリを確認する。
- **解決済み値**（`Resolver`）— *スロットの関数*であるものは `make` や compose ファイルではなくここで導出する: `DB_LOCAL` / `DB_TEST`、app 層の compose プロジェクト、mock-auth の issuer URL、`INFRA_NO_RECREATE`、そして 2 つの worktree を互いの stream から引き離す Realtime Delivery の名前（`REALTIME_TABLE_SUFFIX` / `REALTIME_QUEUE_PREFIX` / `REALTIME_TOPIC`）。Realtime の名前には消費者が 2 つある — app へ渡す `docker-compose.attach.yaml` と、その名前の table を落とす `make realtime-reset` — ので、まさにそれゆえ導出を 2 度書いてはならない。`DB_LOCAL` も同じ経路で override へ届く（`DB_NAME` として）ため、app コンテナはスロットファイルが持つ生値 `DB_NAME_LOCAL` ではなく解決値から自分のデータベース名を得る。いずれもレジストリがリースを確認したスロット番号から導き、`.gobp-db-slot` に並んでいる文字列は読まない。その 3 つが継ぐ基底名は埋め込み `env/.env` だけから取り（`LoadRealtimeBase`）、プロセスの環境変数とは混ぜない。`db-slot env` は同じ 3 変数を撒くため、読み戻す resolver は、出力を export したシェルで 2 度目に走ったときに既にスロットを継いだ名前へさらに継いでしまう（`local_wt2` → `local_wt2_wt2`）。application の「実行時 env を優先する」規則（`config.Load`）が意図的に当てはまらない唯一の場所である。空の基底は素通しせず拒否する。`realtimeName` は空の基底にスロットを継がないため、撒かれる値も表示される値も主 checkout の資源を指してしまうからである。override 側も空を拒む（`:?`）が、そちらのエラーは compose の補間段のもので、埋め込み env のどのキーが欠けているかへは辿り着けない。1 つの導出が `db-slot env`（`make` が eval する値）と `db-slot status`（人間が読む値）の両方を賄うため、表示される値と実際に使われる値が食い違うことはない。`DB_LOCAL` / `DB_TEST` だけは `.gobp-db-slot` を読む `make` 変数としても保持する。target-specific な代入（`db-local-migrate-up: DB=$(DB_LOCAL)`）はパース時に評価され、どのレシピよりも先に決まるからである。
- **所有権ガード**（`RequireOwner`）— `make require-db-owner` の実体で、「リースを持たないリンク worktree」を検出する。判定は `.gobp-db-slot` の存在ではなくレジストリによる。依拠する区別は 2 値ではなく 3 値である。**git が無い**（ツールランナーのコンテナ）と**リポジトリでない**は素通りし、**リポジトリではあるのに構成を読み取れない**場合は失敗する。理由は [`docs/maintenance/db-worktree-pool.md`](../../../docs/maintenance/db-worktree-pool.md) を参照。
...
- **`Resolver`**（判断ロジック） — スタブ `GitProbe` に対する単体テスト。1 台のマシンから git の 4 文脈すべてに到達する唯一の方法である。実機は常にそのうちのちょうど 1 つでしかなく、危険なケース（git が読み取れないリポジトリ）は望んだときに再現できない。所有権の分岐についても同様にスタブ `LeaseProbe` を使い、保持・回収済み・確認不能のそれぞれへ、実体のレジストリを用意せずに到達する。
