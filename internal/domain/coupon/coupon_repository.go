...
type Repository interface {
	...
	// LockByID は、引き換え・返却のために ID からクーポンを取得します。存在しない場合は NotFound を返します。
	...
	// UpdateUnused は、使用済みのクーポンを未使用へ戻します。対象は LockByID で取得し
	// [Coupon.Restore] が戻せると判定したものです。既に未使用だった場合は ErrNotUsed を返します。
	UpdateUnused(ctx context.Context, id uuid.UUID) error
}
