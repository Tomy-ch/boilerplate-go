...
**被検査側が所有する完全な宣言は allowlist ではありません。** ここからは見分けが付きにくいので言明しておきます。allowlist は検査する側に置かれ、検査が見逃すことに同意した対象を列挙するので、陳腐化しても何も落ちません。宣言は説明する対象のコードの隣に置かれ、例外ではなくその対象を網羅し、宣言自体が双方向で検査されます — 陳腐化した項目も、未分類の項目も、どちらも落ちます。各 `Build*` の隣にある `payload_parity.yaml` が現行の実例で、これは payload を検査から除外するものではなく、その payload が何を約束したのかを検査が読むためのものです。
