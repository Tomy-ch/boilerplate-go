...
#
# `-v` は付けていないので出力は 1 パッケージ 1 行になる。1 件落ちたときでも通過分の `ok` 行が
# パッケージ数ぶん残り、読む側（特にエージェント）はその全部をコンテキストへ載せることになる
# ので、通過行と no test files 行だけを捨てる。
#
...
# カンマ連結して渡すため、go test はカバレッジを報告するたびに、その全リストを
# `of statements in ...` として行末へ複製する。
...
# 行頭アンカーが外れて ok 行が残るうえ、接頭辞自体も行を太らせる。時刻の手前の `[^0-9]*` は
# BOM を吸うため。ローカルの go test 出力にも TAB はあるが、2 つ目の TAB の後が日付に
# ならないので当たらない。
#
...
# 1 行が大きく膨らまないための保険で、削除ではなく省略なので判断材料は失われない。
#
