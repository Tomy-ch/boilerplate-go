...
| `make realtime-reset` | Brings up `dynamodb_local`, then runs `scripts/realtime-reset` from the host to delete this checkout's three tables and wait until each is gone. Creation stays with `realtime-provision`. An empty `-endpoint` is refused, because it would fall through to the SDK's default resolution and reach real DynamoDB. | Run by `slot-acquire` so a slot's PostgreSQL and its EventLog are rebuilt together — see [db-worktree-pool.md](../docs/maintenance/db-worktree-pool.md). Also useful by hand when a local stream is wedged behind a stale sequence |
