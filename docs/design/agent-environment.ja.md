...
決定できる性質は機械的に失敗させる。`depguard` と architecture test は層境界を、generator と drift check は派生成果物を、対象を絞った lint は workflow と文書の規約を保護する。読解を要する判断は [ADR-0097](../adr/0097-multi-model-adversarial-review.md) の finder-to-verifier で独立 review に残す。
...
## 人間が送ったことに意味が依存するシグナル

人間がそれを作ることを選んだからこそ重みを持つ成果物がある。したがってエージェントが同一の成果物を生成すると、その重みは供給されるのではなく壊れる。他リポジトリの issue への public なクロスリファレンスがその実例である。それは実在のプロジェクトがその issue を注視し解決を必要としていることを上流のメンテナへ伝え、彼らは優先順位付けの際にそれを重み付けする。エージェントが issue を起票し参照を大量に集められるようになった今、ツールが発した参照はメンテナが送ることを選んだものと見分けが付かず、その件数はシグナルからスパムへ劣化する。

この種のシグナルに対する統制は、より良く情報を得たエージェントによって満たされることがなく、包括的な自律の付与によって移転もされない。[AGENTS.md](../../AGENTS.md) がエージェントに評価可能な規則ではなく都度の人間の確認を要求し、既定を上流に痕跡を残さない形にしているのはこのためである。
