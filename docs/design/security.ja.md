...
通常の PR は報告し、昇格 PR がゲートします。論拠は [ADR-0093](../adr/0093-multi-layer-security-scanning.ja.md) にありますが、要点はこうです。既存の依存ツリーから受け継いだ脆弱性は、その PR が持ち込んだものではなく、その PR の作者に直せるものでもありません。そこでブロックすれば、上流の修正が別のどこかで入るまで無関係な変更がすべて赤くなります。予想される帰結は、チェックの無効化か、赤いまま merge する習慣の定着です。どちらもゲートの価値を上回るコストです。
...
**バージョン参照は同一性ではない。** タグは付け替えられ、mutable な image タグは再ビルドされます。したがってバージョンは人間が読む意図としてソースに残し、不変の digest を SSOT である lockfile が持ちます——`.github/actions-pin.toml`（[ADR-0094](../adr/0094-sha-pinned-actions.ja.md)）と `docker/images-pin.toml`。どちらの検査も fail-closed で、未固定と未登録は別のエラーとして扱われ、片方がもう片方に化けることはありません。
...
**image の経過日数も同じ向きに倒す。** マルチアーキのタグは index を指し、その各プラットフォームの manifest はそれぞれ固有の image config `created` を持ちます。`pin-images` はそのうち**最も新しい**ものを採ります。反対に最も古いものを採ると、タグを「この image の何らかの形はいつから存在するか」として読むことになりますが、検疫が問うているのはそこではありません——lockfile へ書かれるのは index の digest であり、1 プラットフォームを足す・作り直すだけで、それ以前には存在しなかった参照が生まれ、汚染が見つかるだけの時間をまだ経ていないからです。攻撃側のコストも 2 つの向きで大きく違います。`created` は image config の自己申告値で、レジストリの署名が覆う範囲にありません——最も古いを採る側に対しては、自分が足した 1 プラットフォームの日付を古く偽装するだけで index 全体の検疫を抜けられますが、最も新しいを採る側に対しては全プラットフォームを同時に古く見せる必要があります。

**これは窓を狭めるだけで、`created` を信用できるようにするものではありません。** プラットフォーム間で値が揃っている場合——再現ビルドが `created` を固定時点（`SOURCE_DATE_EPOCH`、多くは Unix epoch）へ釘付けしている場合——最も古いと最も新しいは同じ値を返し、どちらの向きでも「どの窓でも超えられない経過日数」が報告されます。向きが決めるのは、プラットフォーム間で値が食い違ったときの挙動だけです。
...
帰結が 3 つあります。はじめの 2 つは、解除がツリーへ**到達する**ことについて事後監査が要らない理由になります。記録されずに通るものが無いので、検知すべき対象がそもそも存在しません。3 つ目はその論法が止まる場所です——レビューを通って入った免除も、期限を越えて生き延びることはでき、一度レビューされたことは見直されることを何も意味しません。
...
**例外は期限つきの債務で、その期限は両方向に効きます。** 窓が開く前に消せば install がすべて落ち、版がカットオフを越えて熟成すればエントリは不要になり、無くても lockfile は通ります。したがって各エントリには、何を免除しているか・理由・そのパッケージがどこで動くか・いつ削除できるようになるかを記録します。そしてその日付は散文ではなく機械可読にします。人しか読まない日付は、誰も読み返さない日付だからです。置き場は [`.github/pnpm-cooldown-bypass.toml`](../../.github/pnpm-cooldown-bypass.toml) で、`go-cooldown` / `tool-cooldown` のバイパスファイルと同じ形です。[`scripts/pnpm-cooldown`](../../scripts/pnpm-cooldown)（`make pnpm-cooldown-check`）は、期限の無い例外・期限切れや 3 ヶ月より先を指す期限・どの例外にも対応しないエントリ・lockfile がもう解決していない版のいずれでも失敗します。期限を `pnpm-workspace.yaml` の外に置くのは意図的で、それが pnpm にとって意味を持たず pnpm が読むファイルを占める筋合いが無いこと、そしてそこを読む検査はコメント解釈になり、pnpm が honor するのに解釈側が取りこぼす書き方が「免除ゼロ」として通ってしまうことによります。pull request だけでなくスケジュールでも走るのは、期限がどちらのファイルの変更も無しに訪れるためです。これは pnpm が自前で必要とした唯一の検査でした——解決器が窓を守っていた一方で免除は誰も守っておらず、このゲートができるまで 2 件が記載の削除期限を 23 日と 27 日過ぎたまま残っていました。
...
Python のツールは、`mise.toml` が全ツールのバージョンを持つという原則に対する唯一の例外で、その理由は供給網側にあります。PyPI ツールはバージョンを固定してもほとんど何も固定できません——依存ツリーが install 時に解決されるため、同じ pin でも日によって別のツリーが入ります。そこで各ツールは、バージョンを `python/<tool>.in` で宣言し、解決結果——推移依存すべてと sha256 ハッシュ——を `python/<tool>.txt` が持ちます（[ADR-0084](../adr/0084-mise-ssot-drift-gate.ja.md)。仕組みは `python/README.md`）。install は常に `uv pip install --require-hashes -r <tool>.txt` で、バージョンかハッシュを欠く要求は拒否されるため、完全性の検証は install の一部であり、飛ばせる別工程ではありません。
...
**URL 内のクレデンシャルは transport だけでなくログの問題でもある。** Realtime Delivery は SSE ストリームを**クエリパラメータ**で運ぶ ticket で認証します。`EventSource` が `Authorization` ヘッダを送れないためです（[ADR-0074](../adr/0074-query-ticket-stream-authentication.ja.md)）。この選択は、クレデンシャルを、インフラが既定でコピーするリクエストの部分——URL——へ移します。プロセス内での答えはログと span 属性からそれを削ることですが、それが届くのは自分のプロセスが書くものだけです。load balancer・proxy・CDN はそれぞれ独自にクエリ文字列をログするため、stream の経路ではデプロイ側がそれを除外またはマスクしなければなりません（[realtime-delivery.ja.md §4.2](realtime-delivery.ja.md#42-deployment-側)）。これが許容できる理由となる緩和策は、ticket が短命で単一の宛先に紐づき単一目的である——ストリームを認証するだけで何も認可しない——こと、そして明示的に失効可能なことです。なお stream の security scheme は OpenAPI ドキュメント上の第二の authenticator でもあるため、上の規則はそれにも当てはまります——`security` 要求を書き忘れた operation は無防備で、それを捕まえるのは spec の差分レビューです。
