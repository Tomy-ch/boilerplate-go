...
[ADR-0032 (lightweight-cqrs)](../adr/0032-lightweight-cqrs.ja.md)（構築物とその構造、CommandService の
eligibility と derivation の規則）、
[ADR-0033 (system-cqrs-dml-category)](../adr/0033-system-cqrs-dml-category.ja.md)（分割の外側にある
第 4 のカテゴリ）、
...
ドメイン集約との 1:1 対応が壊れる。[ADR-0033](../adr/0033-system-cqrs-dml-category.ja.md) を参照。DML は
...
### 門 2 —— Indivisibility: 非機能要件がその書き込みの分解を禁じるか

独立した 3 つの問いが決める。読み、境界を跨ぐ書き、書き込みの形そのものという別々のことを問うため
独立しており、1 つの操作が複数に「はい」と答えることもある。
...
3. 書き込む行を**identity で名指しできるか** —— それとも述語でしか決まらないため、1 集約ずつ分解すると
   往復が母集団に比例して増えてしまうか。前の 2 つと異なり、この問いは境界を跨ぐかどうかに一切依存
   しないため、単一の集約内に留まる書き込みを含め、この門に到達するすべての書き込みに問われる。

手順:

1. **分解（既定）。** 他集約への帰結が結果整合でよく、そこから読んだ条件が陳腐化してよいなら、通常の
   usecase として実装し、帰結を outbox イベントとして伝播する
   （[ADR-0054](../adr/0054-transactional-outbox.ja.md)）。他の集約をトランザクション内に保持しない。
2. **Guard（同期的な行ロック。通常の usecase のまま）。** §5 を参照。
3. **Indivisibility（CommandService。例外であり、正当化を要する）。** 到達する経路は 2 つあり、
   独立している —— 操作はどちらか一方だけでも到達し得る:
   - **(3a) 複数集約の原子性。** 複数集約*書き込み*の単一トランザクション原子性が要求として残る
     場合。この経路はトランザクション境界を広げ、§9 がその逸脱を記録する。
   - **(3b) 述語で決まる集合書き込み。** 書き込む行を identity で名指しできないため、列挙もロックも
     できず、上限も無い。分解すると行ごとに 1 集約を再構築することになり、往復が母集団に比例して
     増える。これは §3.3 の書き込み側の鏡像である —— 読み取り側は、分解がその操作に不要な集約を
     実体化しないかを問い、こちらは行ごとに 1 つを実体化しないかを問う。**トランザクション境界は
     広げない**。書き込みは単一の集約内に留まってよく、3b は何個の集約に触れるかについて何も述べない。
     [ADR-0114](../adr/0114-predicate-defined-set-writes-on-commandservice.ja.md) で決定。

受け入れられない正当化が 2 つある。「複数集約を跨ぐから CommandService」と、「読むだけだから何も要らない」。
3 つ目も認められない —— **「集合が大きいから CommandService」** である。3b が問うのは行を*名指しできるか*
であって、件数ではない。identity で名指しできる 1000 行はそれでも分解できるし、述語で選ばれた 3 行は
分解できない。
...
行ロックを取る（[ADR-0036](../adr/0036-ordered-pessimistic-row-locks.ja.md)）。規則が集約を跨ぐ場合は
...
Guard の枝（§5）と、**分岐 3a** によって到達する CommandService（§4 門 2）は、どちらも複数の集約に属する行を
単一トランザクション内に置くため、[`internal/domain/README.md`](../../internal/domain/README.ja.md)
（§ Aggregate Boundary）が述べる原則から逸脱する。認めるのはちょうどこの 2 つの拡張だけであり、他は認めない。
理由は [ADR-0034](../adr/0034-commandservice-atomicity-criterion.ja.md) の § Departure from "1 Aggregate = 1
Transaction Boundary" に記録されている。

**分岐 3b は 3 つ目の拡張ではない。** これは別の問い —— 書き込みが集約を跨ぐかではなく、行を名指しできるか
—— によって同じ構成要素へ到達し、この分岐が認める書き込みは 1 つの集約にしか触れなくてよい。したがって
CommandService の存在はもはやトランザクション境界が広げられた証拠にはならない —— その証拠になるのは分岐 3a
である（[ADR-0114](../adr/0114-predicate-defined-set-writes-on-commandservice.ja.md)）。
