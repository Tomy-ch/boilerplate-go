...
# ADR-0093: 多層セキュリティスキャン——報告とゲートを分離し、ハードニングされたランナー上で行う
...
| cooldown より新しい依存バージョン | Go cooldown + tool cooldown（[ADR-0095](0095-malicious-package-detection-via-cooldown.ja.md)。pnpm は依存解決の時点で自前の窓を課す） | `go.mod` / `mise.toml` / `python/*.in` の変更 / 週次 | 同一ワークフロー（direct な Go の require とツール宣言） |
...
[ADR-0094](0094-sha-pinned-actions.ja.md) の SHA pin と隔離期間を通す。
...
- ここに挙げたスキャナのうち pre-commit でも走るのは zizmor だけ。フックと CI が呼ぶ `make`
  ...
  （[ADR-0089](0089-local-hooks-mirror-ci.ja.md)）。開発者が
