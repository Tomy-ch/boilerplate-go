...
[ADR-0032](0032-lightweight-cqrs.ja.md) は CommandService という構成要素の
...
CommandService が要る」という素朴なヒューリスティクスは、都合がよいときに集約境界を
広げることになり、都合で広がる境界は何も制約しない。関連する曖昧さは ADR-0032
...
並行する書き込みがそれを無効化し得る（[ADR-0036](0036-ordered-pessimistic-row-locks.ja.md)）。
...
2. **その複数集約書き込みは単一トランザクションの原子性を要件として要求するか？**
   ...
   理由である。
3. **書き込む行を identity で名指しできるか？** 述語でしか決まらない対象を持つ書き込みは、列挙も
   ロックもできず、上限も無いため、分解すると行ごとに 1 往復を発行することになる。
   [ADR-0114](0114-predicate-defined-set-writes-on-commandservice.ja.md) で追加された。本 ADR は、
   境界を跨ぐ書き込みだけでなくすべての書き込みに問われるという理由で、この問いには到達
   しなかった。
...
残る場合（分岐 3a）—— または、ADR-0114 により、書き込む行が述語でしか名指しできない場合（分岐 3b）
に限る。認められない正当化が 2 つある。「複数の集約にまたがるから CommandService」と、分岐 2 が
...
分岐 2 と分岐 3a はどちらも、複数の集約に属する行を単一のトランザクションに入れる。
...
- **本リポジトリが代わりに行うこと。** 名前の付いた 2 つの拡張、そしてその 2 つだけ。
  ...
  走る（分岐 3a）。CommandService へ至るもう 1 つの経路 —— 書き込む対象行が述語でしか
  名指しできない書き込みである分岐 3b
  （[ADR-0114](0114-predicate-defined-set-writes-on-commandservice.ja.md)）—— は 3 つ目の
  拡張では**ない**。この分岐が認める書き込みは 1 つの集約内に留まってよい。読み手に
  とっての帰結は、CommandService の存在がどの境界が跨がれたかをもはや示さなくなる
  ということである —— それを示すのは分岐 3a である。
- **理由。** Evans の議論は *変更* についてのものである。広い境界の害は、読み込んだ
  ...
  対する唯一の権威であり続ける。分岐 3a は複数を変更するので狭い例外に留める。要件が
  「中間状態が観測されてはならない」と述べる場合にのみ認め、そこで強制される条件も
  ドメインが著述する（[ADR-0032](0032-lightweight-cqrs.ja.md) § Derivation）。
...
- **購入作成 — ガードは分岐 2、書き込みは分解できる。** 在庫の検証と減算は購入確定と原子的で
  なければならない。「在庫がないのに購入が成功した」状態は、一瞬たりとも観測されては
  ならない（売り越し禁止）。だがその原子性を与えるのは usecase が既に所有している
  トランザクションであって CommandService ではない。書き込む行は identity で名指しできる
  （明細が参照する商品）ため、先に id 昇順でロックしたうえで
  `product.Repository.UpdateStock` と `purchase.Repository.Create` へ分解できる。
  これは下の廃番の例と対で読む実例である。3 つの集約に触れてなお、数は何も決めない。
  これとは独立に、同じトランザクションは購入者が在籍していることをガードしており、この条件は
...
- **ユーザー退会 — ガードは分岐 2、カスケードは分岐 1。** 退会の中核は
  ...
  分岐 3a は当たらず、操作は usecase のままである。しかし並行する購入作成がそれを
...
- **商品の廃番 — 分岐 3a。原子性によってこの分岐に到達する実例。** 廃番は商品を非公開にし、その商品を
  カートに入れていた全員へ代償のクーポンを発行する。受給者は述語 — `cart_items` から `carts` への
  結合と、退会済みの除外 — でしか決まらないため、id で名指しできず、ロックも張れず、件数に上限も
  無い。分解すれば受給者ごとに `Coupon` 集約を再構築することになり、これは
  `docs/design/data-access-pattern.md` §3.3 の書き込み側の鏡像である。決め手になる要件は
  「クーポンが配られているのに商品がまだ買える」状態が観測されてはならないこと。クーポンは商品を
  失った代償なので、両方を同時に持てる窓が開くのは誤りである。したがって商品の書き込みと一括発行は
  CommandService を通して同一トランザクションに乗る。
  **上の購入作成と対で読むこと。** あちらも 3 つの集約に触れてなお分解できる。行を id で名指しできる
  からである。数は同じで、答えが違う。
  この実例が意図して「そうしていない」ことが 2 つある。カートの明細は**取り除かない** — 明細を
  残して `discontinued` の issue を立てるほうが理由が見える形で残るし、非公開化そのものは上の実例の
  とおり分岐 1 なので、削除は論拠を何も担わない。そして実装が使う 2 つの文（受給者の取得と一括挿入）は
  **逸脱ではない**。集合演算の基準は往復が母集団に比例して増えないことであって、文がちょうど 1 つで
  あることではない。分かれるのは主キーの採番をドメインに置く
  [ADR-0037](0037-uuidv7-identifiers.md) の要請による。仕様は `docs/spec/usecase/product.md`。
  **この実例は両方の問いに「はい」と答える。** だからこそ 1 つの分岐だけでこれほど長く運べていた。
  行は述語で決まるため、分岐 3b だけでもこれを認める。分岐 3a を決め手にしているのは商品側の
  書き込みである。下の販促の一括発行と読み比べてほしい。あちらは 3b にしか答えない。

- **販促の一括発行 — 分岐 3b。原子性なしでこの分岐に到達する実例。**
  退会していない全ユーザーへ販促クーポンを 1 枚ずつ発行する操作は `coupons` だけを書き、他には何も
  書かない。第二の集約を書かないため分岐 3a は一切発火しない —— 2 つの集約にまたがる中間状態を
  観測者が捉える余地はなく、この操作のどの部分も即時である必要がない。それでも分解はできない。
  受給者は `users` に対する述語で決まるため、identity で名指しできず、ロックも張れず、件数にも
  上限が無い。書き込みを load-mutate-save として表現すれば、受給者ごとに `Coupon` を構築し
  受給者ごとに 1 往復を発行することになる。これが分岐 3b であり、この操作が CommandService で
  ある理由のすべてである。
  **上の廃番と読み比べてほしい。** どちらも述語によってクーポンを一括発行するが、トランザクション
  境界を広げるのは一方だけである。この対は、かつて 1 つの分岐 3 が同時に答えていた 2 つの問い ——
  「これらの書き込みはコミットを共にしなければならないか」と「これらの行は名指しできるか」——
  を分けて見せている。
  この実例が意図して *していない* ことが 2 つある。配布の実施を記録**しない** —— キャンペーンを
  実行したという印は何も残らないため、この操作を 2 回実行すれば全員に 2 回発行してしまう。廃番には
  自然な再実行ガードとして `IsDiscontinued` があるのに対し、こちらには何も無く、重複実行への防御は
  もっぱら呼び出し側の `Idempotency-Key` に委ねられている。そして受給者数の上限は**ドメイン不変
  条件ではない** —— それは書き込みの前に usecase が検査するアプリケーションポリシーであり、
  CommandService はここでもドメインが著述していない条件を強制しない
  （[ADR-0032](0032-lightweight-cqrs.ja.md) § Derivation）。
  [ADR-0114](0114-predicate-defined-set-writes-on-commandservice.ja.md) で決定。仕様は
  `docs/spec/usecase/coupon.md`。

- **クーポンの引き換え — 分岐 1。ロックは、このトランザクション自身が書く行に対して取る。** 購入確定で
  ...
  **このクーポンのロックは分岐 2 のガードではない。** ガードは何も書かず、観測を無効化する書き手を
  ...
  （[ADR-0036](0036-ordered-pessimistic-row-locks.md)）であり、購入作成における商品ロックと同じ役割で
  ある。あちらもガードとは呼んでいない。このワークフローにおけるガードは購入者の行であり、それは上の
  購入作成が既に記録している。
  **上の廃番と読み比べること。** どちらもクーポン集約を書くのに、判別式の反対側へ落ちる。
  分けているのは跨ぐ集約の数でも操作の重要さでもなく、対象の行を名指しできるかどうかである。
  この対を無関係な 2 機能ではなく 1 つの集約の中に置いたのはそのためだ。
  仕様は `docs/spec/usecase/purchase.md`。
...
- 分岐 2 はガード対象の行のスループットを代償にする。その行に対する並行する書き手の
  ...
  （[ADR-0036](0036-ordered-pessimistic-row-locks.ja.md)）。
...
判定は単純になるが、書き込みが触れる集約の数は、その書き込みが分解できるかどうかを何も
語らない。購入作成は 3 つに触れてなお分解できる。対象行が identity で名指しでき、ロックを
張れるからである。分解を禁じるのは、述語でしか決まらず列挙もロックもできない対象集合の方だ。
数で分類すれば、境界を広げるのが都合のよいときに CommandService を認めることになり、
都合で広がる境界は何も制約しない（「1 集約 = 1 トランザクション境界」からの逸脱 節）。却下。
...
[ADR-0036](0036-ordered-pessimistic-row-locks.ja.md) が記録するインターリーブである。
...
（[ADR-0036](0036-ordered-pessimistic-row-locks.ja.md) § 5）、書き込み最適化の構成要素の
ものではない。
...
[ADR-0036](0036-ordered-pessimistic-row-locks.ja.md) が記録するとおりである。分離レベルは
...
- **[ADR-0032](0032-lightweight-cqrs.ja.md) との関係**: 補完関係であり、衝突しない。
  ...
  しない。
- **[ADR-0036](0036-ordered-pessimistic-row-locks.ja.md) との関係**: 本 ADR は集約横断の
...
<!-- sample-api:replace-end -->
- 関連: [ADR-0033](0033-system-cqrs-dml-category.ja.md)（CQRS 分割の外側で outbox の
  DML を担う `system_cqrs` カテゴリ）、
  [ADR-0054](0054-transactional-outbox.ja.md)（DB への書き込みと外部エンドポイントへの
  publish の間の dual-write anomaly を解く outbox パターン。分解したカスケードを*配送する*
  手段であって、単一 DB の内側で分解を正当化するものではない）、
  [ADR-0035](0035-transaction-retry-idempotent-callers.ja.md)（原子的書き込みが走る
