...
# ADR-0096: インフラ統合テストはリアル DB に対してセンチネルエラーロールバックで実行する
...
- ローカル開発と CI の両方で PostgreSQL インスタンスが実行されている必要がある。これによりテスト環境にインフラ依存関係が追加される（`make go-test` 前に `make db-init` を実行しなければならない）。
