...
- `make go-test`(カバレッジ付き)を実行する。カバレッジは現行 baseline から **低下させない**。新規 / 変更パッケージは **90%** 超、handler は ~100% に近づける。
- ラインを下回るパッケージは不足分岐テストを追加する — 満たすまで止めない。(「完了」の定義は [`rules.md` → Testing & Definition of Done](rules.ja.md)。)
- **カバレッジ % は代理指標であって目的ではない。** テストの価値は動かした数値ではなく、退行に対して固定した契約である。有意味なテストが 0% しか足さないことはある — 空の no-op default は被覆可能な文を持たないので、`{}` の本体に出る関数全体の `0.0%` は Go の表示上の産物であってギャップではない。それを追いかけてはならず、カバレッジを上げないことを理由に実契約を検証しているテストを削ってもならない。逆に、意味のある assert を伴わずラインに到達するためだけに走らせるコードは coverage theater であり、そちらが無意味である。
...
以下のバーが適用される前に、テストは固定する価値のある契約を持っていなければならない。テストが有意味であるのは、それが守る契約(正しさ / 不変条件 / 境界 / 安全性)が (1) 実際に退行し得る、(2) 他所で既に固定されていない、(3) 対象の層が所有している、の 3 つを満たすときに限る。無意味な形はその裏返しである。

- **意味論の誤り** — トートロジー、付随的な実装詳細への assert、カバレッジを動かすためだけに存在するケース。
- **冗長な重複** — 同じ経路を新しい観点なしに層をまたいで 2〜3 回検証する。依存や生成コードを再テストする。
- **層の誤り** — 対象の層が所有していない関心事を検証する。
...
- **自動修正される壊れた入力** — `make go-fix` が黙って直してしまう不正リテラル。ケースは通り続けるのに拒否経路を検証しなくなる。フィールド名の綴り誤り(`"nmae"`)は `misspell` が実在名へ書き換えるため、拒否されるべき本文が正当な本文に変わる。どの linter にも「直せない」不正さを選ぶこと。未知フィールドなら、綴りは正しいが契約に無い名前を使う。
