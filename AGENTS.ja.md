...
黙って 1 つを選ばないこと。「三つとも回しますか」と尋ねるのも駄目です —— 値付けをしないまま
...
3 つは **対等であり、互いを起動しません。** 1 つの対象に 1 つのスキルであり、そのスキルがその
対象を監査する唯一の場所です。`/impl-review` はテストのレンズもコメントのレンズも持たず、他の
2 つは `/impl-review` が走るかどうかに関わらず独立に起動されます。結合は **問いかけの側** に
留め、スキルの中には入れないでください。**これはパイプラインの中でも成り立ちます** —— issue を
マージ済み PR まで駆動するスキルも、そのレビューフェーズでこの 3 つを尋ね、ユーザーの代わりに
選ばないこと。
