...
**ノイズの多いターゲットは `ai-<target>` で実行する。** `make ai-lint` は `make lint` を実行し、
出力をすべて `tmp/ai-logs/lint.txt` へ退避します。成功時は何も表示せず、終了コードはそのまま
返り、失敗したときだけログの場所を 1 行で示すので、壊れたところだけを読めます。既定でこうする
理由は、そうしなければコマンドの出力が全文あなたのコンテキストへ読み込まれ、しかも*失敗した*
コマンドは残りを取り戻すファイルを渡されないまま抜粋されるためです。切り落とされるのは、
まさにそれを実行した目的である診断です。出力そのものが答えであるターゲット（`help` /
`load-status` / `base-branch`）や、完了しないターゲット（`serve` / `worker` / `outbox-relay`）
には使いません。規約と `make clean-ai-logs` は `.makefiles/README.md` にあります。
